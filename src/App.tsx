...
function App() {
  ...
  const loadStories = useCallback(async (cat: Category, pageNum: number) => {
    setLoading(true);
    setError(null);
    try {
      const ids = await fetchStoryIds(cat);
      const start = pageNum * 20;
      const end = start + 20;
      const pageIds = ids.slice(start, end);
      const fetchedStories = await Promise.all(pageIds.map(id => fetchStory(id)));
      setStories(fetchedStories.filter(Boolean) as Story[]);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        {!loading && stories.length > 0 && (
          <div className="flex justify-center gap-4 mt-8 mb-6">
            ...
            <button
              ...
              className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            >
              次へ →
            </button>
          </div>
      ...
      <footer className="text-center py-6 text-gray-500 dark:text-gray-400 text-sm">
        <p>Powered by Hacker News API | Render対応</p>
      </footer>
    </div>
  );
}
